...
class NakamaRepository @Inject constructor(
	...
	suspend fun authenticateWithGoogle(googleIdToken: String, username: String): Boolean =
		withContext(Dispatchers.IO) {
			try {
				val url =
					"${config.value.httpBaseUrl}/v2/account/authenticate/google?create=true&username=${username.filter { it.isLetterOrDigit() }}"
				...
				if (response.isSuccessful) {
					...
					Log.w(
						"NakamaRepository",
						"Google Auth failed on server with code ${response.code}, falling back to Custom Auth"
					)
	...
	suspend fun authenticateWithEmail(
		...
		try {
			val sanitizedUsername =
				username.filter { it.isLetterOrDigit() }.ifBlank { "Player${(1000..9999).random()}" }
			val url = if (isSignUp) {
	...
	private fun authenticateWithDeviceInternal(username: String): Boolean {
		return try {
			...
			val urlDevice =
				"${config.value.httpBaseUrl}/v2/account/authenticate/device?create=true&username=$sanitizedUsername"
			...
			if (!response.isSuccessful) {
				...
				val urlCustom =
					"${config.value.httpBaseUrl}/v2/account/authenticate/custom?create=true&username=$sanitizedUsername"
						...
						if (state == 0) {
							list.add(
								NakamaFriend(
									...
									displayName = user.optString(
										"display_name",
										user.optString("username", "Friend")
									),
	...
	private fun connectWebSocketAndQueueMatchmaker(
		friendFilter: String? = null,
		isRetry: Boolean = false,
		useWsPath: Boolean = false
	) {
		try {
			...
			webSocket = httpClient.newWebSocket(request, object : WebSocketListener() {
				override fun onOpen(ws: WebSocket, response: Response) {
					...
					val msg = JSONObject().apply {
						...
						put("matchmaker_add", JSONObject().apply {
							...
							put(
								"query",
								if (friendFilter != null) "properties.friend:'$friendFilter'" else "*"
							)
						})
					}
				...
				override fun onFailure(ws: WebSocket, t: Throwable, response: Response?) {
					Log.e(
						"NakamaRepository",
						"Nakama WebSocket failed on path $path: ${t.message}, HTTP code: ${response?.code}"
					)
					...
					if (!isRetry && (response?.code == 404 || response?.code == 401 || t is java.net.ProtocolException)) {
						Log.w(
							"NakamaRepository",
							"WebSocket token rejected (HTTP ${response?.code}). Re-authenticating with device..."
						)
	...
	private fun handleIncomingWsMessage(text: String) {
		try {
			...
			if (json.has("matchmaker_matched")) {
				...
				val payload =
					if (dataBase64.isNotEmpty()) String(Base64.decode(dataBase64, Base64.DEFAULT)) else "{}"
